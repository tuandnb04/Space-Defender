...
{
    [Header("Scroll Settings")] public float scrollSpeed = 1.8f;

    public bool fitCameraWidth = true;

    [Header("Background Sprites")] public Transform background1;
    ...
    private void Update()
    {
        ...
        if (background1) background1.position += Vector3.down * movement;

        if (background2) background2.position += Vector3.down * movement;

        if (!background1 || !background2) return;
        // When background1 moves completely below camera view
        if (background1.position.y <= -_bgHeight)
            background1.position = new Vector3(background1.position.x, background2.position.y + _bgHeight,
                background1.position.z);

        // When background2 moves completely below camera view
        if (background2.position.y <= -_bgHeight)
            background2.position = new Vector3(background2.position.x, background1.position.y + _bgHeight,
    ...
    public void SetupBackgroundDimensions()
    {
        ...
        if (cam != null && fitCameraWidth && background1 != null)
        {
            ...
            if (sr1 != null && sr1.sprite != null)
            {
                ...
                if (background2 != null) background2.localScale = new Vector3(scaleX, scaleY, 1f);
            }
        }
